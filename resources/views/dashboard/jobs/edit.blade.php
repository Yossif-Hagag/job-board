    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 container">
            ...
            <div class="bg-white dark:bg-gray-900 shadow-md rounded-xl p-8">
                <form method="POST" action="{{ route('dashboard.jobs.update', $job) }}" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $job->title) }}" required
                                class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white rounded-lg shadow-sm">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" rows="4" required
                                class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white rounded-lg shadow-sm">{{ old('description', $job->description) }}</textarea>
                        </div>
                    </div>
                            ...
                            <select name="category_ids[]" multiple required
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $job->categories->pluck('id')->contains($category->id) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="location_ids[]" multiple
                                ...
                                @foreach ($locations as $location)
                                    <option value="{{ $location->id }}"
                                        {{ $job->locations->pluck('id')->contains($location->id) ? 'selected' : '' }}>
                                        {{ $location->city }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="language_ids[]" multiple required
                                ...
                                @foreach ($languages as $language)
                                    <option value="{{ $language->id }}"
                                        {{ $job->languages->pluck('id')->contains($language->id) ? 'selected' : '' }}>
                                        {{ $language->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @foreach ($attributes as $attribute)
                                <div>
                                    ...
                                    <input type="text" name="attr_{{ $attribute->id }}"
                                        value="{{ old('attr_' . $attribute->id, optional($job->attributeValues->firstWhere('attribute_id', $attribute->id))->value) }}"
                                        class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white rounded-lg">
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="flex justify-between mt-3">
                        ...
                        <button type="submit"
                            class="border px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow">
                            🔄 Update
                        </button>
                    </div>
                </form>
